...
@Singleton
class GetCoursesUseCase @Inject constructor(private val homeRepository: UdemyRepository,
                                            private val localRepository: LocalRepository
) {

    suspend operator fun invoke(page: Int, pref: Prefs): Resource<CoursePage>{
        return try {
            //Get the filters from saved
                val requestCourses = RequestCourses(pref.category,
                   page.toString(),
                   "20",
                   pref.price,
                   pref.learningLevel,
               ...
               val courseDtos = homeRepository.getCourses(requestCourses)
